...
\graphicspath{ {./images/} }
\usepackage{url}
\usepackage[paper=a4paper,left=2cm,right=2cm,top=2cm,bottom=3cm]{geometry}
...
\pagestyle{headings}
\titlepic{\includegraphics[scale=1]{cover}}
\date{}
\title{}
\begin{document}
	\thispagestyle{empty}
...
\clearpage
	...
	\subsubsection{Dialog State Tracking (DST)}\label{dst}
...
&P\big(s_{t+1}|o_t ,s_t\big) = P\big(s_{t+1} | o_t,a_t,s_t \big)\cdot P\big(o_t | a_t,s_t\big)\cdot P\big(a_t | s_t\big) \nonumber\\
	...
	\begin{figure}[H]
		\hspace*{-2.25cm}   
		\centering
		\includegraphics[scale=0.73]{plot_reinforce_250.pdf}
